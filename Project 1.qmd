...
Issue fixed; updated as follows. Function is getPUMSv2.1c

```{r}
# WT: issue related to mutate() is fixed. Function is getPUMSv2.1c
...
Above code was modified as follows. Function is choosecatv2
...
WT: The above function is being updated as follows. Function is getPUMSv3b
...
Default call: https://api.census.gov/data/2022/acs/acs1/pums?get=SEX,PWGTP,MAR&SCHL=24&key=YOUR_KEY_GOES_HERE To get the region-specific data: https://api.census.gov/data/2022/acs/acs1/pums?get=SEX,PWGTP,MAR&for=region:*&SCHL=24&key=YOUR_KEY_GOES_HERE. To get the division-specific data: https://api.census.gov/data/2022/acs/acs1/pums?get=SEX,PWGTP,MAR&for=division:*&SCHL=24&key=YOUR_KEY_GOES_HERE To get the state-specific data: https://api.census.gov/data/2022/acs/acs1/pums?get=SEX,PWGTP,MAR&for=state:\*&SCHL=24&key=YOUR_KEY_GOES_HERE
...
WT: above code was updated as follows. Function getPUMSv4.0b
...
getPUMSv4.1b <- function (year = 2022, numvarinput = "AGEP", catvarinput = "SEX", geolvlinput) {
  base_url <- paste0 ("https://api.census.gov/data/", as.character(year), "/acs/acs1/pums")
...
```{r}
# WT: updated the above function as follows. Function "getPUMSv4.2b", still can't set geolvlinput default as ALL. This code does not work for geography level filtering
getPUMSv4.2b <- function (year = 2022, numvarinput = "AGEP", catvarinput = "SEX", geolvlinput, filter_value = NULL, filter_column = NULL) {
  base_url <- paste0 ("https://api.census.gov/data/", as.character(year), "/acs/acs1/pums")
...
getPUMSv4.2b(2022,"GASP","HHL","ST", filter_value = "20", filter_column = "SCHL")
# following not working
getPUMSv4.2b(2022,"GASP","HHL","ST", filter_value = "32", filter_column = "ST")

```

It throws out an error. Looks like geographic-related variables could not be subsetted in the same way as other variables. But how to solve the 'for' problem? We figured out later a backtick (\`) should be used to avoid confusion.

```{r}
getPUMSv4.3 <- function (year = 2022, numvarinput = "AGEP", catvarinput = "SEX", geolvlinput = "ALL", geospecinput) {
  base_url <- paste0 ("https://api.census.gov/data/", as.character(year), "/acs/acs1/pums")
  
  query_params <- list(
    get = paste0 (choosenum(numvarinput)[1],",",choosenum(numvarinput)[2],",",choosecat(catvarinput)),
    `for` = paste0 (choosegeolvl(geolvlinput),":",geospecinput),
    SCHL = "24",
    key = "41b1ae36641c0a01c8cf9a48396a6c82ab8f6d9c")
  ...
  PUMScust_parsed <- fromJSON(rawToChar(PUMScust$content))
  PUMScust_tibble <- as_tibble (PUMScust_parsed)
  return (PUMScust_tibble)
}
getPUMSv4.3(2022,"GASP","HHL","state","09")
getPUMSv4.3(2022,"GASP","HHL","division","1")
getPUMSv4.3(2022,"GASP","HHL","region","1")
```

Code above.. try one more time... Function "getPUMSv4.3b"

```{r}
# WT: updated the above function as follows. Function "getPUMSv4.3b", still can't set geolvlinput default as ALL. Not working
getPUMSv4.3b <- function (year = 2022, numvarinput = "AGEP", catvarinput = "SEX", geolvlinput = "REGION", geospecinput) {
  base_url <- paste0 ("https://api.census.gov/data/", as.character(year), "/acs/acs1/pums")
...
getPUMSv4.3b(2022,"GASP","HHL","ST","48")
```
...
getPUMSv4.4 <- function (years = c(2022), numvarinput = "AGEP", catvarinput = "SEX", geolvlinput, filter_value = NULL, filter_column = NULL) {
  ...
  # change the column vector to 'factor' type without adding a column to the dataset
  PUMScust_tibble <- PUMScust_tibble %>% mutate(!!choosecat(catvarinput) := as.factor(.[[choosecat(catvarinput)]]))
  PUMScust_tibble <- PUMScust_tibble %>% mutate(!!choosenum(numvarinput)[1] := as.numeric(.[[choosenum(numvarinput)[1]]]))
...
getPUMSv4.4(c(2021,2022),"GASP","HHL","ST",filter_value="24", filter_column = "SCHL" )

```

In this function "getPUMSsum", we get the class 'census' assigned to tibble at the end, so that the next function 'summary.census' will provide a method specifically for the census class.
```{r}
...
getPUMSsum <- function (years = c(2022), numvarinput = "AGEP", catvarinput = "SEX", geolvlinput, filter_value = NULL, filter_column = NULL) {
  ...
  # this is to add a column and change the value type to numeric
  PUMScust_tibble <- mutate(PUMScust_tibble, PWGTP_n = as.numeric(PUMScust_tibble$PWGTP))
  # change the column vector to 'factor' or 'numeric' type without adding a column to the dataset
  PUMScust_tibble <- PUMScust_tibble %>% mutate(!!choosecat(catvarinput) := as.factor(.[[choosecat(catvarinput)]]))
  PUMScust_tibble <- PUMScust_tibble %>% mutate(!!choosenum(numvarinput)[1] := as.numeric(.[[choosenum(numvarinput)[1]]]))
...
Below is a function "summary.census" that produces means and standard deviations for numeric variables and counts for categorical variables
```{r}
...
Next, create a generic plot() function for a census class tibble
```{r}
...
plot.census <- function(tibble_data, cat_var, num_var){
  plot <- ggplot((tibble_data), aes(x = get(cat_var), y = get(num_var), weight = PWGTP_n)) + geom_boxplot() +
  labs(title = paste("Plot of", num_var, "by", cat_var)) + xlab(cat_var) + ylab(num_var)
  print(plot)
}
